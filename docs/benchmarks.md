...
*Refreshed automatically by `benchmarks.yml` after every merge to `main`.
...
> **Note:** Cold-start latency (~30s) applies only on the first request after
> Railway free-tier sleep. All numbers above are steady-state after warm-up.
